...
/** Task */
gulp.task(TASK_NAME, () => {
  return gulp.src(config.paths.lint.styles.from)
    // Error handler
    .pipe($.plumber({
...
gulp.task(WATCH_TASK_NAME, () => {
  return gulp.watch(config.paths.lint.styles.watch, [TASK_NAME])
    .on('unlink', (filepath) => {
